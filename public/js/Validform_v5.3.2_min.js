			...
			if (q && !a.util.isEmpty.call(d(this), k) && !n) {
				...
				var p = {
					...
					success : function (x) {
						...
						}else if(d.trim(x.status) == 'repeat'){
							m[0].validform_valid = "true";
							//console.log( m.attr("sucmsg"));
							//console.log(v.data("tipmsg").r);
							//console.log( e.r, o.tiptype);
							//console.log(x.info);
							//console.log(typeof JSON.parse(x.info));
							var repeatData = JSON.parse(x.info);
							//console.log(repeatData);
							$("#qq").val(repeatData['qq']);
							$("input[name='stuname']").val(repeatData['stuname']);
							if(repeatData['stusex'] ==  1){
								$("#male").attr('checked','checked');
							}else{
								$("#female").attr('checked','checked');
							}
							$("input[name='email']").val(repeatData['email']);
							$("#aa"+repeatData["cintent"]).attr('checked','checked');
							$("#a"+repeatData["clientsource"]).attr("checked","checked");
							$("#b"+repeatData["visitsource"]).attr("checked","checked");


							$("select[name='intentionschool']").find($("option[value='"+repeatData['intentionschool']+"']")).attr("selected",true);
							$("select[name='intentionschool']").trigger("change");
							$("select[name='intentionsubject']").find($("option[value='"+repeatData['intentionsubject']+"']")).attr("selected",true);
							$("select[name='intentionsubject']").trigger("change");
							$("select[name='classid']").find($("option[value='"+repeatData['classid']+"']")).attr("selected",true);
							


							$("textarea[name='consultrecord']").html(repeatData['consultrecord']);
							$("textarea[name='remark']").html(repeatData['remark']);
							$("form").attr('action','/index.php/client/repeatInsert');
							$("input[name='button']").val('提交修改');
							a.util.showmsg.call(v, m.attr("remsg") || v.data("tipmsg").r || e.r, o.tiptype, {
								obj : m,
								type : 2,
								sweep : o.tipSweep
							}, "bycheck");
